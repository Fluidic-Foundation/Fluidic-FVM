...
This is an early research testnet. State may reset and there are no real funds — do not use real money or production secrets.
...
By default this runs as a **light client**: it follows the operator mesh, verifies synthesis certificates, and exposes the API, but does not synthesize or stake. The node discovers peers via the public BitTorrent Mainline DHT and LAN mDNS, so an explicit `PEERS` list is optional.

To run a synthesizing **full operator**, advertise a `PUBLIC_ENDPOINT` and set `SYNC_PEERS` to an existing node's HTTP API before synthesizing, otherwise your state roots diverge from the network. For example:
...
docker run -d --name fluidic-node \
  ...
  -p 8080:8080 -p 7000:7000 \
  -e OSCILLATOR_ID=12345 \
  -e FLUIDIC_CLIENT_MODE=false \
  -e PUBLIC_ENDPOINT=tcp://1.2.3.4:7000 \
  -e SYNC_PEERS=https://api.testnet.fluidic.foundation \
  -e FLUIDIC_DATA_DIR=/data \
  -v "$HOME/fluidic-data:/data" \
  ghcr.io/fluidic-foundation/fluidic-fvm/mesh-node:latest
```

If you have been running older/broken builds, delete `$HOME/fluidic-data/snapshot.json` and restart with a fresh snapshot to avoid stale state.
...
| `BOOTSTRAP_DNS` | `''` | Optional DNS domain with signed genesis TXT bootstrap records |
| `BOOTSTRAP_URL` | `''` | Optional HTTPS URL serving signed JSON bootstrap records |
...
| `FLUIDIC_PSK` | `''` | Hex-encoded 32-byte key for encrypted mempool decryption |
| `GOSSIP_REQUIRE_AUTH` | `false` | When `true`, peers must prove `FLUIDIC_PSK` to gossip. Default `false` keeps the public testnet permissionless. |
...
2. The node decides its role:
   ...
   - **Full operator** (when `PUBLIC_ENDPOINT` is advertised or `FLUIDIC_CLIENT_MODE=false`): connects to `SYNC_PEERS`, downloads the current state, and only then stakes genesis balance and synthesizes.
3. It discovers peers through the public BitTorrent Mainline DHT and LAN mDNS, optional signed genesis bootstrap records (DNS TXT / HTTPS), and any explicit `PEERS`. The API server starts independently of peer discovery so `/api/health` is available quickly.
4. It joins the gossip mesh.
