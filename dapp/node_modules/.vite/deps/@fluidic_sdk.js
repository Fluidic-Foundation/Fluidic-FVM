...
// node_modules/@fluidic/sdk/dist/utils.js
...
function u64ToLeBytes(n) {
  ...
  for (let i = 0; i < 8; i++) {
    buf[i] = Number(n >> BigInt(i * 8) & BigInt(255));
  }
  return buf;
}
function u128ToLeBytes(n) {
...
function i128ToLeBytes(n) {
  let value = n % BigInt(2) ** BigInt(128);
  if (value < 0n)
    value += BigInt(2) ** BigInt(128);
  const buf = new Uint8Array(16);
  for (let i = 0; i < 16; i++) {
    buf[i] = Number(value >> BigInt(i * 8) & BigInt(255));
  }
  return buf;
}
...
function padHex32(hex) {
  const clean2 = hex.startsWith("0x") ? hex.slice(2) : hex;
  if (clean2.length > 64)
    throw new Error("hex longer than 32 bytes");
  return clean2.padStart(64, "0");
}
function ensureHex32(value) {
  const clean2 = padHex32(value);
  return clean2.toLowerCase();
}
function hashBlake3(data) {
  return blake3(data);
}
function accountIdFromPublicKey(publicKey) {
  return bytesToHex2(blake3(publicKey));
}
function deriveAccount(base, salt) {
  return bytesToHex2(blake3(concatBytes2(new TextEncoder().encode("fluidic:derived-account:v1"), hexToBytes2(ensureHex32(base)), new TextEncoder().encode(salt))));
}
function waveAddressFromAccountId(accountId) {
  return bytesToHex2(blake3(concatBytes2(new TextEncoder().encode("fluidic:wave-address:v1"), hexToBytes2(ensureHex32(accountId)))));
}
function evmAddressToFluidicAccount(evmAddress) {
  const clean2 = evmAddress.startsWith("0x") ? evmAddress.slice(2) : evmAddress;
  if (clean2.length !== 40)
    throw new Error("EVM address must be 20 bytes");
  return bytesToHex2(blake3(concatBytes2(new TextEncoder().encode("fluidic:evm-account:v1"), hexToBytes2(clean2))));
}
function coordinateToBytes(coord) {
  const buf = new Uint8Array(32);
  for (let i = 0; i < 4; i++) {
    const c = coord.components[i];
    for (let j = 0; j < 8; j++) {
      buf[i * 8 + j] = Number(c >> BigInt(j * 8) & BigInt(255));
    }
  }
  return buf;
}
...
// node_modules/@fluidic/sdk/dist/crypto.js
var FluidicKeypair = class _FluidicKeypair {
  ...
  static generate() {
    ...
    return new _FluidicKeypair(priv, pub);
  }
  /** Derive a keypair from a 32-byte seed. */
  static fromSeed(seed) {
    const bytes = typeof seed === "string" ? hexToBytes2(ensureHex32(seed)) : seed;
    if (bytes.length !== 32) {
      throw new Error("Seed must be 32 bytes");
    }
    const pub = ed25519.getPublicKey(bytes);
    return new _FluidicKeypair(bytes, pub);
  }
  /** Reconstruct from a secret key (public key is derived). */
  static fromSecretKey(secretKey) {
    const bytes = typeof secretKey === "string" ? hexToBytes2(ensureHex32(secretKey)) : secretKey;
    const pub = ed25519.getPublicKey(bytes);
    return new _FluidicKeypair(bytes, pub);
  }
  get publicKeyHex() {
  ...
  toJSON() {
    return {
      publicKey: bytesToHex2(this.publicKey),
...
// node_modules/@fluidic/sdk/dist/shifts.js
var DEFAULT_DEX_DOMAIN = "4445585f574156455f5553444300000000000000000000000000000000000000";
function vectorClockBytes(vc) {
  const entries = Object.entries(vc.entries).map(([node, time]) => ({ node: ensureHex32(node), time })).sort((a, b) => a.node > b.node ? 1 : -1);
  const parts = [];
  for (const { node, time } of entries) {
    parts.push(hexToBytes2(node));
    parts.push(u64ToLeBytes(time));
  }
  return concatBytes2(...parts);
}
function buildCommutativeShift(params) {
  const domain = params.domain ?? DEFAULT_DEX_DOMAIN;
  const nonce = params.nonce ?? 0n;
  const timestampNs = params.timestampNs ?? nowNs();
  const signingBytes = concatBytes2(new TextEncoder().encode("FLUIDIC:COMMUTATIVE:v2"), hexToBytes2(ensureHex32(domain)), coordinateToBytes(params.coordinate), i128ToLeBytes(params.delta), hexToBytes2(ensureHex32(params.poolId)), u64ToLeBytes(nonce), u64ToLeBytes(timestampNs));
...
function buildStatefulShift(params) {
  const from = params.signer.accountId;
  const domain = params.domain ?? DEFAULT_DEX_DOMAIN;
  const predecessors = params.predecessors ?? [];
  const nonce = params.nonce ?? 0n;
  const timestampNs = params.timestampNs ?? nowNs();
  const signingBytes = concatBytes2(new TextEncoder().encode("FLUIDIC:STATEFUL:v2"), hexToBytes2(ensureHex32(domain)), hexToBytes2(ensureHex32(from)), hexToBytes2(ensureHex32(params.to)), u128ToLeBytes(params.amount), vectorClockBytes(params.vectorClock), concatBytes2(...predecessors.map((h) => hexToBytes2(ensureHex32(h)))), u64ToLeBytes(nonce), u64ToLeBytes(timestampNs));
  const signature = params.signer.sign(signingBytes);
  return {
    domain,
    from,
...
function buildRegistrationShift(params) {
  return {
    account: params.signer.accountId,
    public_key: bytesToHex2(params.signer.publicKey),
...
function buildStakeShift(params) {
  const operator = params.signer.accountId;
  const publicKey = bytesToHex2(params.signer.publicKey);
  const nonce = params.nonce ?? 0n;
  const timestampNs = params.timestampNs ?? nowNs();
  const signingBytes = concatBytes2(new TextEncoder().encode("FLUIDIC:STAKE:v1"), hexToBytes2(ensureHex32(operator)), hexToBytes2(ensureHex32(publicKey)), u128ToLeBytes(params.amount), u64ToLeBytes(nonce), u64ToLeBytes(timestampNs));
  const signature = params.signer.sign(signingBytes);
  return {
    operator,
...
function hashStatefulShift(shift) {
  const signingBytes = concatBytes2(new TextEncoder().encode("FLUIDIC:STATEFUL:v2"), hexToBytes2(ensureHex32(shift.domain)), hexToBytes2(ensureHex32(shift.from)), hexToBytes2(ensureHex32(shift.to)), u128ToLeBytes(shift.amount), vectorClockBytes(shift.vector_clock), concatBytes2(...shift.predecessors.map((h) => hexToBytes2(ensureHex32(h)))), u64ToLeBytes(shift.nonce), u64ToLeBytes(shift.timestamp_ns));
  const sig = hexToBytes2(shift.signature);
  return bytesToHex2(hashBlake3(concatBytes2(signingBytes, sig)));
}
function hashCommutativeShift(shift) {
  const signingBytes = concatBytes2(new TextEncoder().encode("FLUIDIC:COMMUTATIVE:v2"), hexToBytes2(ensureHex32(shift.domain)), coordinateToBytes(shift.coordinate), i128ToLeBytes(shift.delta), hexToBytes2(ensureHex32(shift.pool_id)), u64ToLeBytes(shift.nonce), u64ToLeBytes(shift.timestamp_ns));
  const sig = hexToBytes2(shift.signature);
  return bytesToHex2(hashBlake3(concatBytes2(signingBytes, sig)));
}

// node_modules/@fluidic/sdk/dist/quorum.js
var QuorumTracker = class {
  constructor() {
    __publicField(this, "latestQuorumTick", 0);
  }
...
function isFinalized(response) {
  return response.finalized;
}
function parseQuorumResponse(response) {
  return {
    finalized: response.finalized,
    tick: response.tick,
    stake: response.stake ? BigInt(response.stake) : void 0,
    threshold: response.threshold ? BigInt(response.threshold) : void 0,
    roots: response.roots
  };
}

// node_modules/@fluidic/sdk/dist/client.js
var FluidicClient = class {
  constructor(options) {
    __publicField(this, "apiUrl");
    __publicField(this, "wsUrl");
    __publicField(this, "minTickMode");
    __publicField(this, "timeoutMs");
    __publicField(this, "retries");
    __publicField(this, "quorumTracker");
    __publicField(this, "ws", null);
    __publicField(this, "snapshotListeners", []);
    __publicField(this, "latestLocalTick", 0);
    this.apiUrl = options.apiUrl.replace(/\/$/, "");
    this.wsUrl = options.wsUrl ?? this.apiUrl.replace(/^http/, (m) => m === "https" ? "wss" : "ws") + "/api/ws";
  ...
  async fetchJson(path, options = {}, requireMinTick = true) {
  ...
  async balance(account) {
    return this.fetchJson(`/api/account/${account}/balance`);
  }
  /** Fetch the status of a submitted shift. */
  async shiftStatus(hash) {
    return this.fetchJson(`/api/shift/${hash}/status`);
  }
  /** Submit a commutative shift. */
  async submitCommutative(shift) {
    return this.postJson("/api/shift/commutative", {
      coordinate: {
  ...
  async submitStateful(shift) {
    return this.postJson("/api/shift/stateful", {
      from: shift.from,
      to: shift.to,
      amount: shift.amount.toString(),
      vector_clock: Object.fromEntries(Object.entries(shift.vector_clock.entries).map(([k, v]) => [
        k,
        Number(v)
      ])),
  ...
  async operatorInfo() {
    return this.fetchJson("/api/operator/info");
  }
  /** Fetch the list of currently staked operators. */
  async stakedOperators() {
    return this.fetchJson("/api/operators");
  }
  /** Fetch the synthesis certificate for a tick, if any. */
  async certificate(tick) {
    try {
      return await this.fetchJson(`/api/certificate/${tick}`);
    } catch (e) {
  ...
  async getQuorum(tick) {
    const res = await this.fetchJson(`/api/quorum/${tick}`);
  ...
  async waitForFinalization(hash, options = {}) {
  ...
  async waitForQuorum(tick, options = {}) {
    const pollMs = options.pollMs ?? 500;
    const deadline = Date.now() + (options.timeoutMs ?? 3e4);
    while (Date.now() < deadline) {
  ...
  connectWs() {
    if (typeof WebSocket === "undefined") {
      void import("./browser-4AB3X37Q.js").then((mod2) => {
        this.ws = new mod2.default(this.wsUrl);
        this.bindWs();
      });
      return;
    }
  ...
  bindWs() {
    ...
    this.ws.onmessage = (event) => {
      try {
        const snap = JSON.parse(typeof event.data === "string" ? event.data : event.data.toString());
        if (snap.commutative_applied !== void 0) {
    ...
    this.ws.onclose = () => {
      ...
      if (this.snapshotListeners.length > 0) {
        setTimeout(() => this.connectWs(), 1e3);
      }
    };
  }
};

// node_modules/@fluidic/sdk/dist/swap.js
async function submitSwap(client, params) {
  ...
  return {
    ...
    payoutHash: ""
    // server-generated; query via /api/shift/:hash/status
  };
}
function quoteSwap(direction, amount, waveReserve, usdcReserve) {
  if (waveReserve === 0n || usdcReserve === 0n) {
    throw new Error("Pool reserves are empty");
  }
  const amountOut = direction === "WAVE_TO_USDC" ? amount * usdcReserve / waveReserve : amount * waveReserve / usdcReserve;
  const priceImpact = Number(amountOut) / Number(amount);
  return { amountIn: amount, amountOut, priceImpact };
}

// node_modules/@fluidic/sdk/dist/constants.js
var FLUIDIC_EVM_CHAIN_ID = 990492;
var FINALIZATION_DEPTH = 6;
export {
  DEFAULT_DEX_DOMAIN,
  FINALIZATION_DEPTH,
  FLUIDIC_EVM_CHAIN_ID,
  FluidicClient,
  FluidicKeypair,
  QuorumTracker,
  accountIdFromPublicKey,
  buildCommutativeShift,
  buildRegistrationShift,
  buildStakeShift,
  buildStatefulShift,
  ...
  deriveAccount,
  ensureHex32,
  evmAddressToFluidicAccount,
  hashBlake3,
  hashCommutativeShift,
  hashStatefulShift,
...
@noble/curves/esm/ed25519.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=@fluidic_sdk.js.map
